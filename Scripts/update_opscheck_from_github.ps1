...
function Resolve-DownloadSpec {
  if ($AccessMode -eq "private") {
    if ([string]::IsNullOrWhiteSpace($GitHubToken)) {
      throw "Private mode requires a GitHub token. Please pass -GitHubToken."
    }
    return @{ 
...
try {
  ...
  # ★GitHubから引っ張ってくるファイルのリスト
  $files = @(
    "poc_health_report.ps1",
    "run_health_report_hidden.vbs",
    "run_updater_hidden.vbs",
  ...
  foreach ($rel in $files) {
    # GitHubリポジトリのルート直下、または scripts/ フォルダ内の両方を探す
  ...
  Write-Host "[INFO] アップデート完了。バックアップ先: $backupDir"
} catch {
  $failed = $true
  $runStatus = "Error"
